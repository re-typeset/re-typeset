...
bool LettersWriter::write(const QString text, QRect area, int color, Direction direction, bool writePart)
{
    if( direction == Horizontal ) {
        return writeH( text, area, color, writePart );
    } else {
        return writeV( text, area, color, writePart );
    }
}

bool LettersWriter::writeH(const QString text, QRect area, int color, bool writePart)
{
...
bool LettersWriter::writeV(const QString text, QRect area, int color, bool writePart)
{
    int textLength = calculateLength(text);
    int bottomSpace = 0;
